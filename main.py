...
# Prompt otimizado para respostas concisas
SYSTEM_PROMPT = """
...
# Sugestões categorizadas para banco de dados
QUICK_SUGGESTIONS = {
    "🛠️ SQL & Consultas": [
    ...
    for category, suggestions in QUICK_SUGGESTIONS.items():
        with st.expander(category, expanded=True):
            for label, suggestion in suggestions:
                if st.button(label, key=f"sug_{category}_{label}", 
                           help=suggestion,
                           use_container_width=True):
                    st.session_state.suggestion_clicked = suggestion
                    st.rerun()

# Container fixo no topo (área principal)
with st.container():
    with st.expander("📤 Enviar dados para análise", expanded=True):
        col1, col2 = st.columns([3, 1])
        with col1:
            uploaded_file = st.file_uploader("Selecione um arquivo ou imagem de schema", 
                                          type=["png", "jpg", "jpeg", "csv", "xlsx", "sql", "dump"],
...
# Campo de texto inteligente com placeholder dinâmico
...
if user_input or uploaded_file:
    ...
    if uploaded_file:
        if uploaded_file.type.startswith('image'):
            ...
            st.session_state.chat_history.append({"role": "user", "type": "text", "content": f"Analise este arquivo {uploaded_file.type}: {uploaded_file.name}"})
    ...
    with st.spinner("Processando sua consulta..."):
        try:
            # Prepara mensagens com TODO o histórico
            ...
            # Gera resposta com configuração para objetividade
            ...
            st.session_state.chat_history.append({"role": "assistant", "type": "text", "content": f"🔍 Erro: {str(e)}. Por favor, reformule sua pergunta."})
